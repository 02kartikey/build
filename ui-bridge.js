...
// Guard against null — staff-menu-wrap is absent on assessment and counsellor pages.
document.addEventListener('click', function(e) {
  var wrap = document.getElementById('staff-menu-wrap');
  if (!wrap || wrap.contains(e.target)) return;
  var dd = document.getElementById('staff-dropdown');
  var ch = document.getElementById('staff-chevron');
...
function submitCounsellorQuery() {
  ...
  if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
    var emailEl = document.getElementById('cq-email');
    if (emailEl) emailEl.style.outline = '2px solid red';
    return;
  }
  ...
  fetch('/api/counsellor-query', {
    ...
    .catch(function(err) {
      // Re-enable the button so the student can retry — previously it stayed
      // disabled forever on network failure, silently trapping the student.
      if (btn) { btn.disabled = false; btn.textContent = 'Send Request'; }
      var errEl = document.getElementById('cq-error');
